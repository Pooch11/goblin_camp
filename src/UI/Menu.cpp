...
Menu* Menu::OrdersMenu() {
	if (!ordersMenu) {
		...
		boost::function<bool(Coordinate, Coordinate)> checkDitch = std::bind(Game::CheckTileType, TILEDITCH, _1, _2);
		boost::function<void(Coordinate, Coordinate)> rectCall = std::bind(Game::FillDitch, _1, _2);
		...
		boost::function<bool(Coordinate, Coordinate)> checkTree = std::bind(Game::CheckTree, _1, Coordinate(1,1));
		...
		boost::function<void(Coordinate)> call = std::bind(&Game::StartFire, Game::Inst(), _1);
...
Menu* Menu::DevMenu() {
	if (!devMenu) {
		...
		boost::function<bool(Coordinate, Coordinate)> checkTree = std::bind(Game::CheckTree, _1, Coordinate(1,1));
		boost::function<void(Coordinate)> call = std::bind(&Game::CreateFilth, Game::Inst(), _1, 100);
		...
		boost::function<void(Coordinate, Coordinate)> rectCall = std::bind(&Game::RemoveNatureObject, Game::Inst(), _1, _2);
