...
void ConstructionDialog::Expand() {
	if (construct.lock()) {
		boost::function<void(Coordinate, Coordinate)> rectCall = std::bind(&Stockpile::Expand, boost::static_pointer_cast<Stockpile>(construct.lock()), _1, _2);
		boost::function<bool(Coordinate, Coordinate)> placement = std::bind(Game::CheckPlacement, _1, Coordinate(1,1), 
